...
gender = {
    "female": [
        ...
        "The person",
        "This person",
    ],  # in the picture, in the image, the entire face of
...
def get_caption(img_attribute, num_caption):  # pylint: disable=R0914
    """
    ...
    for _ in range(num_caption):
        ...
        if ALL_ATTRIBUTE:
            ...
            caption = f"The person in the picture is {IsAttributes}. \
                {choice(subject)}{choice(HaveVerb)}{HasAttributes}. \
                    {choice(subject)}{choice(WearVerb)}{WearAttributes}."
            ...
            WearAttributes = (
                ", ".join(sample(feature["WearAttributes"], c_w)).replace("Wearing_", "").replace("_", " ").lower()[::]
            )
            ...
            caption_format = {
                ...
                "3": f"This is a {IsAttributes} person. \
                    {choice(subject)}{SelectHasAttributes}. {choice(subject)}{SelectWearAttributes}.",
            }
...
if __name__ == "__main__":
    ...
    for num in range(0, num_images):
        final_captions = get_caption(image_attributes[f"{num}.jpg"], NUM_CAPTION)

        with open(f"{save_path}/{str(num)}.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(final_captions))
